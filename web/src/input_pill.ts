    ...
    const funcs = {
        ...
        appendValidatedData(item: InputPillItem<T>) {
            ...
            if (item.type === "search_user") {
                ...
                //
                // Ideally this code would live in search files, when
                // we generate `item.display_value`, but we currently
                // use `display_value` not only for visual representation
                // but also for parsing the value a pill represents.
                // In the future we should change all input pills to
                // have a `value` as well as a `display_value`.
                if (item.type === "search") {
